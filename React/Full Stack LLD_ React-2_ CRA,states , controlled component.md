...
So the easiest example for that is a simple counter component. Initially we are not using any Create-React-App because you should be clear with the concepts first.

What are the use cases of react(algorithm), react-dom(library actually changes the UI) and Babel (transpiler converts JSX to JS).

So we are building the counter which consist of two buttons like +, - and the counter in between as shown below.
...
- When the setCount gets called, it first updates the count state variable and then call the component again called as rerendering.
...
What changes will we make in the above code to implement the multiple counter as shown below:
...
<body>
    ...
    <script type = "text/babel">
        ...
        ReactDOM.render(<ParentCounter></ParentCounter>, document.getElementById("root"));
    </script>
...
- The "increment" and "decrement" functions in the "Counter" component updates the count state when the respective buttons are clicked.
...
description: how props and states can be used to give custom behaviour to components.
...
- You can create a complex component by using the multiple independent components.
- use case of state wrt to a component: to make it dynamic
...
Whenever you have any data that changes on your UI you should not be using like this, as we not write anything around the input. In react it is always a good practice that your UI should be changed using your state variable. It means that count should be changed using the useState variable, this input also is changing without the will of the react that is not a good practice.
...
- When we are controlling the UI change of elements like input using state then these components are known as controlled components.
...
We can use Create-React_App or the Vite. But here we will be going ahead with using `vite`.

To set up with vite follow the commands below:  
...
**Thought**: Was React developed for the above simple application of counters? Can't you build these things using the normal HTML, CSS and Javascript? No, there are bigger websites like linkedin, facebook. Problems were there before React, that is why React was introduced. So you cannot depend on html, css because they are not even ideal for creating bigger application.
...
1. **Package.json:**  
which include react, react-dom, and other libraries.
2. **Node_Modules:**  
various libraries and tools like babel, eslint, nodelib etc are present.
3. **index.html:**  
in this, the root div is present, and the rollup is done in it.
4. **Src Folder:**  
all the code would be written in the src folder.
...
The top input box consists of a text input field and a button. The bottom container consists of a list of the added tasks. So, there are two components in this.
...
We will be following the main steps as below:

1. Create static HTML
2. Add Eventlistener
3. Identify dynamic data (the task and list of the tasks)
4. Divide them into independent component
...
function InputBox(props) {
    ...
    // we have provide access to taskArr -> task ARR
    return (
...
1. **addTask**:  
This prop is passed to the "InputBox" component and allows users to add new tasks. It's a function that takes the input value of the task and updates the list of tasks.

2. **tasksArr and handleDelete**:  
These props are passed to the "List" component. "tasksArr" is an array that holds the list of tasks, and "handleDelete" is a function for removing tasks. "List" uses these props to display the tasks and provide the functionality to delete them.
...
- Common state lifted up to the parent.
...
- **Delete Feature:** Delete feature deletes an element from the list. As the user clicks on any of the list item it should be deleted from the UI as well as from the list.
- **Delete Feature Logic:** We can have an event listener on the list items. As the user clicks on the element, the listener should call a function `handleDelete`.
- This constant `handleDelete` function that will take the index of the element that is clicked and delete the element from the TODO List.
- This is parent to child and then child to parent relation.
- On click we have to call `handleDelete` in the List element.

## Gist

Lifting the common state:
